...
def gerar_chave_pessoa(row: pd.Series) -> Optional[str]:
    ...
    Args:
        row: Linha do DataFrame com campos nome_envolvido, data_nascimento
        
    Returns:
        String com chave única ou None se dados insuficientes
    """
    try:
        nome = str(row.get('nome_envolvido', '')).strip().lower()
...
def enriquecer_com_chaves(df: pd.DataFrame) -> pd.DataFrame:
    ...
    - natureza_alvo: Classificação da natureza (DESAPARECIMENTO/HOMICIDIO/CADAVER)
    ...
    # Classifica natureza e papel
    df['natureza_alvo'] = df['natureza'].apply(identificar_natureza_alvo)
    df['papel_pessoa'] = df['papel_envolvido'].apply(identificar_papel_pessoa)
    ...
    print(f"- Registros do grupo-alvo: {total_natureza_alvo:,}")
